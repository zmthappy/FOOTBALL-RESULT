//从天天盈球爬取当前的足彩数据进行分析
const footBallSetRank = require("./footBallSetRank.js");
...
// 降板马
const subLeagueTeam = ["卢顿", "伯恩利", "谢菲联", "科隆", "洛里昂", "克莱蒙"];
// 国家队比赛
const countryMatch = [
  "国际友谊",
  "欧洲杯",
...
async function ttyqFootBallData(toDayTime, callBack, selfData) {
  ...
  let data = await getData(
    `https://www.ttyingqiu.com/static/no_cache/league/zc/jsbf/ttyq2020/jczq/jsbf_${toDayTime}.json?v=${toDay.getTime()}`,
    {},
  );
  ...
  let jcData = await getData(
    `https://www.ttyingqiu.com/static/no_cache/league/zc/jsbf/ttyq2020/jczq/${toDayTime}/oz_407_6.json?v=${toDay.getTime()}`,
    {},
  );
  if (data == 404 || !data) {
    callBack(null, 404);
...
async function invokeAllFunc(item, callBack, toDayTime) {
  ...
  try {
    ...
    await delaySecibd();
    await getSixResultHome(item, obj, callBack);
    await delaySecibd();
    await getSixResultVisited(item, obj, callBack);
    await delaySecibd();
    await getSixResultVisitedVistor(item, obj, callBack);
    await delaySecibd();
  ...
  axois
    .get(url, {
      headers: {
        ...
        "Content-Type": "text/html;charset=UTF-8",
        Cookie:
          "VUID=VUID1684064218919764079; Hm_lvt_f081063fc7e101407949a8005a9b3e56=1688727630,1688829029,1688829061,1688870712; device_uuid=dgb6oImE9qptyGy7jwfaeDN3jN5nt-yqlWtWpTno5j7lw",
      },
    })
    .then(function (response, err) {
      let res1 = response.data;
      obj["betWayList"] = [
        res1.bfMatch.homeAmount,
        res1.bfMatch.drawAmount,
        res1.bfMatch.awayAmount,
      ].join();
    })
    .catch((err) => {
      console.log(err, "err");
...
async function getHomeAndAwayResult(item, obj, callBack) {
  ...
  let res1 = await postData(
    "https://sport.ttyingqiu.com/sportdata/f?platform=web",
    params,
  );
  // res1 = JSON.parse(res1)
  let homeChangCi =
    res1.leagueMatchTeamRankVO.homeTeamWin +
    res1.leagueMatchTeamRankVO.homeTeamDraw +
    res1.leagueMatchTeamRankVO.homeTeamLost;
  let awayChangCi =
    res1.leagueMatchTeamRankVO.awayTeamWin +
    res1.leagueMatchTeamRankVO.awayTeamDraw +
    res1.leagueMatchTeamRankVO.awayTeamLost;
  obj["nowMatchChangCi"] =
    homeChangCi >= awayChangCi ? homeChangCi : awayChangCi;
  ...
  if (res1.list) {
    ...
    for (let i = 0; i <= res1.list.length - 1; i++) {
      if (obj["homeAndAwayMatchResult"].length < 1) {
        ...
        if (
          dayjs(nowDay).diff(dayjs("20" + res1.list[i].matchTime), "months") <=
          18
        ) {
...
async function getThreeSeasonRank(item, obj, callBack) {
  ...
  if (leagueName.indexOf("杯") === -1) {
    ...
    if (
      secondRankHome == res3.footballTeamScoreVoList.length ||
      secondRankHome == 1
    ) {
      homeAveRank = Math.round((secondRankHome + homeAveRank) / 2);
    }
    if (
      secondRankVisit == res3.footballTeamScoreVoList.length ||
      secondRankVisit == 1
    ) {
      awayAveRank = Math.round((secondRankVisit + awayAveRank) / 2);
    }
    ...
    if (subLeagueTeam.indexOf(obj.homeTeam) != -1) {
      ...
      obj["ThreeSeasonAverageHome"] = await getTeamId(
        regReturnHome,
        obj.homeTeam,
        homeAveRank,
      );
    ...
    if (subLeagueTeam.indexOf(obj.visitedTeam) != -1) {
      ...
      obj["ThreeSeasonAverageAway"] = await getTeamId(
        regReturnvisi,
        obj.visitedTeam,
        awayAveRank,
      );
    ...
    console.log(
      obj["ThreeSeasonAverageHome"],
      obj["ThreeSeasonAverageAway"],
      "awayAveRank",
    );
  }
}
...
async function getAllEurOdds(item, obj, callBack) {
  ...
  let res1 = await getData(
    `https://www.ttyingqiu.com/live/matchDetail/ftAicaiAllEuropeOdds?matchId=${item.matchId}&isPrimary=0`,
    {},
  );
  if (!res1) {
  ...
  res1.model.list.forEach((dataItem) => {
    if (dataItem.providerName == "威廉*") {
      obj["firstoddswiilian"] = [
        dataItem.firstWinOdds,
        dataItem.firstDrawOdds,
        dataItem.firstLoseOdds,
      ].join();
      obj["endoddswillian"] = [
        dataItem.winOdds,
        dataItem.drawOdds,
...
async function getSixResult(item, obj, callBack) {
  ...
  obj["homeGOal"] =
    res2.teamNearStatus?.goals + "-" + res2.teamNearStatus?.loseGoals;
}

async function getSixResultHome(item, obj, callBack) {
  const params = {
    Number: 10,
    apiName: "getTeamNearStatusAicaiApi",
    bigSmallProviderId: 2,
  ...
  // res2 = JSON.parse(res2);
  if (!res2) {
    recall(getSixResultHome, item, obj);
    return;
  }
  obj["matchResultHomeInhome"] = "";
  let winNumber = 0;
  let drawNumber = 0;
  let loseNumber = 0;
  for (const item of res2.teamNearStatus.list) {
    if (
      ...
      obj["matchResultHomeInhome"].length <= 5
    ) {
      obj.matchResultHomeInhome += item.amidithion;
      if (item.amidithion == "胜") {
        winNumber += 1;
      } else if (item.amidithion == "平") {
        drawNumber += 1;
    ...
    if (obj["matchResultHomeInhome"].length == 6) {
      break;
    }
  }
  obj["homeSixMatchInhome"] = winNumber + "-" + drawNumber + "-" + loseNumber;
  obj["homeGOalInhome"] =
    res2.teamNearStatus?.goals + "-" + res2.teamNearStatus?.loseGoals;
}
...
async function getSixResultVisited(item, obj, callBack) {
  ...
  obj["visitedGoal"] =
    res4.teamNearStatus?.goals + "-" + res4.teamNearStatus?.loseGoals;
}

// 获取近6场客队客场的赛事结果
async function getSixResultVisitedVistor(item, obj, callBack) {
  const params = {
    Number: 10,
    apiName: "getTeamNearStatusAicaiApi",
    bigSmallProviderId: 2,
  ...
  if (!res4) {
    recall(getSixResultVisitedVistor, item, obj);
    return;
  }
  obj["matchResultVisitVisor"] = "";
  let winNumber = 0;
  let drawNumber = 0;
  let loseNumber = 0;
  for (const item of res4.teamNearStatus.list) {
    if (
      item.leagueName != "球会友谊" &&
      obj["matchResultVisitVisor"].length <= 5
    ) {
      obj.matchResultVisitVisor += item.amidithion;
      if (item.amidithion == "胜") {
        winNumber += 1;
      } else if (item.amidithion == "平") {
        drawNumber += 1;
    ...
    if (obj["matchResultVisitVisor"].length == 6) {
      break;
    }
  }
  obj["visitedSixMatchInvisitor"] =
    winNumber + "-" + drawNumber + "-" + loseNumber;
  obj["visitedGoalInvisitor"] =
    res4.teamNearStatus?.goals + "-" + res4.teamNearStatus?.loseGoals;
}
...
async function getAsia(item, obj, callBack) {
  ...
  res3.list.forEach((asia) => {
    ...
    if (asia.providerName == "36*") {
      obj["365First"] = [
        asia.odds[0].firstHomeWinOdds,
        asia.odds[0].firstHandicap,
        asia.odds[0].firstAwayWinOdds,
      ].join();
      obj["365End"] = [
        asia.odds[0].homeWinOdds,
        asia.odds[0].handicap,
        asia.odds[0].awayWinOdds,
      ].join();
    }
  });
  obj["homeLevel"] = await getQIuDuiRanks(
    obj.ThreeSeasonAverageHome,
    obj.homeTeamLeageaName,
  );
  obj["awayLevel"] = await getQIuDuiRanks(
    obj.ThreeSeasonAverageAway,
    obj.visitedTeamLeageaName,
  );
  ...
  fs.writeFileSync(
    "./src/data/newDataFootball.json",
    JSON.stringify(computeList),
    "UTF-8",
  );
}
...
function handicapDepth(obj) {
  ...
  let realityHandicap = obj["365First"]
    ? convertHandicap(obj["365First"].split(",")[1])
    : "";
  ...
  console.log(footBallSetRank, "footBallSetRank");

  footBallSetRank.main(obj);
}

function isJudgeUpOrDown(
  levelSubPositioning,
  realityHandicap,
  historyHandicap,
) {
...
function getTransformPankou(probilit, obj, type) {
  if (probilit >= 0.25 && probilit <= 0.5) {
    ...
    } else if (probilit >= 0.55 && probilit < 0.6) {
    ...
    } else if (probilit >= 0.85 && probilit < 0.9) {
    ...
    if (probilit > 1.1 && probilit <= 1.2) {
      ...
      obj["shuiwei" + type] = (0.9 - (probilit - 1.45)).toFixed(2);
    }
    ...
    if (probilit > 1.5 && probilit <= 1.74) {
      ...
      obj["shuiwei" + type] = (0.9 - (probilit - 1.95)).toFixed(2);
    }
